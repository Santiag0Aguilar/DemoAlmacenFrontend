...
function ToolForm({ onSubmit, isLoading }) {
  const [form, setForm] = useState({ nombre: '', tipo: '', numeroSerie: '', marca: '', modelo: '', descripcion: '' });
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input className="input" value={form.nombre} onChange={set('nombre')} required placeholder="Ej: Taladro percutor" />
        </div>
        <div>
          <label className="input-label">Tipo *</label>
          <input className="input" value={form.tipo} onChange={set('tipo')} required placeholder="Eléctrica, Manual..." />
        </div>
        <div>
          <label className="input-label">Número de serie</label>
          <input className="input" value={form.numeroSerie} onChange={set('numeroSerie')} placeholder="SN-001" />
        </div>
        <div>
          <label className="input-label">Marca</label>
          <input className="input" value={form.marca} onChange={set('marca')} placeholder="Bosch, DeWalt..." />
        </div>
        <div>
...
export default function ToolsPage() {
  ...
  const { data: tools = [], isLoading } = useQuery({
    ...
    queryFn: () => toolsService.getAll({ search: search || undefined, estado: statusFilter || undefined }),
  });
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{tools.length} herramientas registradas</p>
        </div>
                ...
                {tools.map((tool) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="font-mono text-xs text-slate-500">{tool.numeroSerie || '—'}</td>
                      <td className="text-slate-400 text-xs">{[tool.marca, tool.modelo].filter(Boolean).join(' ') || '—'}</td>
                      <td>
                      ...
                      <td>
                        {active ? (
                          <span className="text-xs text-slate-400">{active.trabajador?.nombre}</span>
                        ) : (
      ...
      {/* Create modal */}
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Registrar Herramienta">
        <ToolForm onSubmit={createMutation.mutate} isLoading={createMutation.isPending} />
      </Modal>
    </div>
  );
}
